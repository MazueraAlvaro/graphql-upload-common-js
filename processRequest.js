...
var _GRAPHQL_MULTIPART_REQUEST_SPEC_URL = _interopRequireDefault(require("./GRAPHQL_MULTIPART_REQUEST_SPEC_URL.mjs"));
var _ignoreStream = _interopRequireDefault(require("./ignoreStream.mjs"));
...
function processRequest(request, response, {
  ...
  return new Promise((resolve, reject) => {
    ...
    let map;
    const parser = (0, _busboy.default)({
      ...
      switch (fieldName) {
        case "operations":
          try {
            ...
            return exit((0, _httpErrors.default)(400, `Invalid JSON in the ‘operations’ multipart field (${_GRAPHQL_MULTIPART_REQUEST_SPEC_URL.default}).`));
          }

          // `operations` should be an object or an array. Note that arrays
          // and `null` have an `object` type.
          if (typeof operations !== "object" || !operations) return exit((0, _httpErrors.default)(400, `Invalid type for the ‘operations’ multipart field (${_GRAPHQL_MULTIPART_REQUEST_SPEC_URL.default}).`));
          operationsPath = (0, _objectPath.default)(operations);
          break;
        case "map":
          {
            if (!operations) return exit((0, _httpErrors.default)(400, `Misordered multipart fields; ‘map’ should follow ‘operations’ (${_GRAPHQL_MULTIPART_REQUEST_SPEC_URL.default}).`));
            let parsedMap;
            try {
              parsedMap = JSON.parse(value);
            } catch (error) {
              return exit((0, _httpErrors.default)(400, `Invalid JSON in the ‘map’ multipart field (${_GRAPHQL_MULTIPART_REQUEST_SPEC_URL.default}).`));
            }

            // `map` should be an object.
            if (typeof parsedMap !== "object" || !parsedMap || Array.isArray(parsedMap)) return exit((0, _httpErrors.default)(400, `Invalid type for the ‘map’ multipart field (${_GRAPHQL_MULTIPART_REQUEST_SPEC_URL.default}).`));
            ...
            for (const [fieldName, paths] of mapEntries) {
              if (!Array.isArray(paths)) return exit((0, _httpErrors.default)(400, `Invalid type for the ‘map’ multipart field entry key ‘${fieldName}’ array (${_GRAPHQL_MULTIPART_REQUEST_SPEC_URL.default}).`));
              map.set(fieldName, new _Upload.default());
              for (const [index, path] of paths.entries()) {
                if (typeof path !== "string") return exit((0, _httpErrors.default)(400, `Invalid type for the ‘map’ multipart field entry key ‘${fieldName}’ array index ‘${index}’ value (${_GRAPHQL_MULTIPART_REQUEST_SPEC_URL.default}).`));
                try {
                  operationsPath.set(path, map.get(fieldName));
                } catch (error) {
                  return exit((0, _httpErrors.default)(400, `Invalid object path for the ‘map’ multipart field entry key ‘${fieldName}’ array index ‘${index}’ value ‘${path}’ (${_GRAPHQL_MULTIPART_REQUEST_SPEC_URL.default}).`));
                }
              }
            }
            resolve(operations);
    ...
    parser.on("file", (fieldName, stream, {
      ...
      if (!map) {
        (0, _ignoreStream.default)(stream);
        return exit((0, _httpErrors.default)(400, `Misordered multipart fields; files should follow ‘map’ (${_GRAPHQL_MULTIPART_REQUEST_SPEC_URL.default}).`));
      }
      ...
      stream.on("limit", () => {
        fileError = (0, _httpErrors.default)(413, `File truncated as it exceeds the ${maxFileSize} byte size limit.`);
        stream.unpipe();
    ...
    parser.once("finish", () => {
      request.unpipe(parser);
      request.resume();
      if (!operations) return exit((0, _httpErrors.default)(400, `Missing multipart field ‘operations’ (${_GRAPHQL_MULTIPART_REQUEST_SPEC_URL.default}).`));
      if (!map) return exit((0, _httpErrors.default)(400, `Missing multipart field ‘map’ (${_GRAPHQL_MULTIPART_REQUEST_SPEC_URL.default}).`));
    ...
    response.once("close", () => {
      released = true;
      if (map) for (const upload of map.values()) if (upload.file)
        // Release resources and clean up temporary files.
        upload.file.capacitor.release();
    });
    request.once("close", () => {
      if (!request.readableEnded) exit((0, _httpErrors.default)(499, "Request disconnected during file upload stream parsing."));
    });
    request.pipe(parser);
  });
}
